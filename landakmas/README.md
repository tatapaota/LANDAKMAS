...
## Status saat ini (update, menggantikan bagian di bawah)

Bagian "SUDAH/BELUM dikerjakan" di bawah ini adalah catatan dari Tahap 1 (baru
konversi tampilan, data masih dummy di JavaScript). Sejak itu project sudah
lanjut jauh melewati tahap tersebut:

- Database MySQL sungguhan sudah terpasang — lihat migration di
  `database/migrations` (`arsips`, `booking_requests`, `admins`,
  `page_visits`, dst) dan seeder di `database/seeders`.
- Form booking, tambah arsip, dan login admin sudah submit beneran ke
  database lewat Controller (bukan lagi simulasi JavaScript).
- Autentikasi admin sungguhan sudah jalan lewat guard `auth:admin`.
- Import data arsip dari Excel sudah ada (pakai `maatwebsite/excel`).
- Notifikasi email untuk riwayat booking sudah terpasang (lihat konfigurasi
  `MAIL_*` di `.env.example` dan `app/Notifications`).

Dump data terkini ada di `database-backup/` di root repo ini kalau butuh
restore atau lihat isi data yang sudah pernah tersimpan.

<details>
<summary>Catatan lama dari Tahap 1 (arsip sejarah, sudah tidak akurat)</summary>
